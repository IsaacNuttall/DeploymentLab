...
const baseURL = 'http://18.220.53.166/api'

const getCompliment = () => {
    axios.get("http://18.220.53.166/api/compliment/")
        .then(res => {
            const data = res.data;
            alert(data);
    });
...
const getFortune = () => {
    axios.get("http://18.220.53.166/api/fortune/")
        .then(res => {
            const data = res.data;
            alert(data);
    });
...
dateBtn.addEventListener('click', updateGoal)
